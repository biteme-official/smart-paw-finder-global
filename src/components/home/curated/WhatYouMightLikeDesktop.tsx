        ...
        <div className="overflow-hidden">
          <div ref={scrollRef} className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
            {CURATED_PANELS.map((panel) => (
              <div key={panel.title} className="flex-shrink-0 w-[calc((100%-2rem)/3)] flex flex-col items-center gap-3">
                <div className={`relative w-full overflow-hidden rounded-xl ${panel.bgClass} aspect-square`}>
                  ...
                  <Link
                    ...
                    className="absolute bottom-7 left-1/2 z-10 -translate-x-1/2 inline-flex items-center rounded-xl bg-background px-6 py-3 text-base font-semibold text-foreground shadow-sm hover:bg-background/80 transition-colors"
                  >
                    Shop the Edit
                  </Link>
                </div>
                <p className="line-clamp-1 w-full text-center text-lg font-bold text-foreground">{panel.title}</p>
              </div>
            ))}
          </div>
        </div>
