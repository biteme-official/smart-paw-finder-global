        ...
        >
          {PROMO_DEFS.map((promo) =>
            isExternalHref(promo.href) ? (
              <a key={promo.title} href={promo.href} target="_blank" rel="noopener noreferrer" className="block h-full w-full flex-shrink-0">
                <img src={promo.pcImage} alt={promo.title} className="h-full w-full object-cover" />
              </a>
            ) : (
              <Link key={promo.title} to={promo.href} className="block h-full w-full flex-shrink-0">
                <img src={promo.pcImage} alt={promo.title} className="h-full w-full object-cover" />
              </Link>
            )
          )}
