...
const ADMIN_SECRET = process.env.ADMIN_SECRET || (_VERCEL_ENV !== 'production' ? '990113' : '');
...
function gaDateRange(range: string, customStart?: string, customEnd?: string): { startDate: string; endDate: string } {
...
function extractDateParams(req: VercelRequest) {
  const range = (req.query.range as string) || '7d';
  const start = req.query.start as string | undefined;
  const end = req.query.end as string | undefined;
  return { range, start, end, dr: gaDateRange(range, start, end) };
}
...
async function handleDashboard(token: string, req: VercelRequest, res: VercelResponse) {
  ...
  const orderQuery = endDate
    ? `created_at:>='${startDate.toISOString()}' created_at:<='${endDate.toISOString()}' financial_status:paid`
    : `created_at:>='${startDate.toISOString()}' financial_status:paid`;
...
async function handleGAOverview(req: VercelRequest, res: VercelResponse) {
  const { dr } = extractDateParams(req);
